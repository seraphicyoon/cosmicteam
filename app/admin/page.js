...
export default function AdminPage() {
  ...
  const stats = useMemo(() => {
    ...
    const reaperturas = pedidos.filter((p) => p.reopen_requested).length;
  ...
  const reabrirChatPorSolicitud = async (pedidoId) => {
    const { error } = await supabase
      .from("orders")
      .update({
        chat_closed: false,
    ...
    if (error) {
      setMensaje("No se pudo reabrir el chat.");
      return;
    }
    ...
    setMensaje("Chat reabierto por solicitud de reposición 💖");
  };

  const ignorarSolicitudReapertura = async (pedidoId) => {
    const { error } = await supabase
      .from("orders")
      .update({
        reopen_requested: false,
    ...
    setMensaje("Solicitud marcada como revisada.");
  };
          ...
          {activeTab === "resumen" ? (
            ...
            >
              {[
                ...
                ["Solicitudes de reapertura", stats.reaperturas],
              ].map(([label, value]) => (
                <div
                  key={label}
                  style={{
                            ...
                            {pedido.reopen_requested ? (
                              ...
                              >
                                <div style={{ fontWeight: "bold", color: "#c5578b" }}>
                                  Solicitud de reapertura pendiente
                                </div>
                                ...
                                >
                                  <button
                                    onClick={() => reabrirChatPorSolicitud(pedido.id)}
                                    style={smallBtn("#4c9a69")}
                                  >
                                    Reabrir chat
                                  </button>
                                  ...
                                  <button
                                    onClick={() => ignorarSolicitudReapertura(pedido.id)}
                                    style={smallBtn("#94456b")}
                                  >
                                    Ignorar
                                  </button>
